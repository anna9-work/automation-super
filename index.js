...
async function insertInventoryLogOut({ branch, sku, warehouseLabel, unitPricePiece, qtyBox, qtyPiece, userId, refTable, refId }) {
  ...
  // ⚠️ 你的 logs 結構目前沒有「倉庫別 / 倉庫代碼」，請先用下列 SQL 補欄位再使用：
  // ALTER TABLE public.inventory_logs ADD COLUMN IF NOT EXISTS "倉庫別" text, ADD COLUMN IF NOT EXISTS "倉庫代碼" text;
  ...
  const row = {
    ...
    '操作來源': 'LINE',
    'user_id': userId || null,
    '倉庫別': warehouseLabel,              // 需先補欄位
    '倉庫代碼': warehouseCode              // 需先補欄位
  };
  ...
  if (parsed.type==='change') {
    ...
    if (parsed.action==='out') {
      ...
      try {
        ...
        // 推 GAS（GAS 端重拉 RPC 覆蓋整頁，確保同日同SKU×倉單列累加）
        const payload = {
          type: 'log',
